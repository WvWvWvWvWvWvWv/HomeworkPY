...
x = input("Выберите раскладку rus или eng: ")
word = input("Введите слово: ").upper()
if x == "eng":
    print(
        f"Вы получаете { sum([k for i in word for k, v in eng.items() if i in v])} очков!"
    )
elif x == "rus":
    print(
        f"Вы получаете { sum([k for i in word for k, v in rus.items() if i in v])} очков!"
    )
